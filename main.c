...
/*   Updated: 2019/09/14 15:51:58 by rgwayne-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		*ft_name(void *inc)
{
	...
	if (!(ft_strcmp(fl->name, "mandelbrot")))
		...
		julia(fl);
	else if (!(ft_strcmp(fl->name, "ship")))
		burningship(fl);
	return (NULL);
}
...
t_glob		*get_fractal(char *name)
{
	...
	fl->i.p = mlx_new_image(fl->mlx_ptr, WT, HT);
	fl->i.dt = (int *)mlx_get_data_addr(fl->i.p, &fl->i.b, &fl->i.lz, &fl->i.k);
	fl->per_bits = 8;
	fl->prb = 1;
	fl->min.re = -2.0;
	fl->max.re = 2.0;
...
int			main(int ac, char **av)
{
	...
	if (ac < 2 || (ft_strcmp(av[1], "mandelbrot")
				&& ft_strcmp(av[1], "julia") && ft_strcmp(av[1], "ship")))
		ft_exit(1);
